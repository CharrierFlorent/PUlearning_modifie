...
from dict_match import get_dico
...
if __name__ == "__main__":
    ...
    for e in range(1, 1000):
        ...
        if e % 2 == 0:
            ...
            if f1_train > f1_best_train:
              ...
              p_train_partial, r_train_partial, f1_train_partial, partial_entity_train, partial_ratio_train = dp.compute_precision_recall_f1_partial(newLabels, newPreds, args.flag, 1,True)  
            ...
            if f1_valid <= trainer.bestResult:
                ...
                p_valid_partial, r_valid_partial, f1_valid_partial, partial_entity_valid, partial_ratio_valid = dp.compute_precision_recall_f1_partial(newLabelsValid, newPredsValid, args.flag,1,True)  
                time = 0
                trainer.save(
                    ("saved_model/{}_{}_{}_lr_{}_prior_{}_beta_{}_gamma_{}_iter_{}").format(args.num, args.dataset, args.flag,
    ...
    pulstmcnn.load_state_dict(
        torch.load("saved_model/{}_{}_{}_lr_{}_prior_{}_beta_{}_gamma_{}_iter_{}".format(args.num, args.dataset, args.flag,
                                                                                      trainer.learningRate,
                                                                                      trainer.m,
                                                                                      trainer.beta,
                                                                                      trainer.gamma, args.iter)))
    ...
    p_valid, r_valid, f1_valid = dp.compute_precision_recall_f1(newLabelsValid, newPredsValid, args.flag,1)
    p_test_partial, r_test_partial, f1_test_partial, partial_entity_test, partial_ratio_test = dp.compute_precision_recall_f1_partial(newLabelsValid, newPredsValid, args.flag,1,True)  
    
    dutils = utils.dict_utils.DictUtils()
    inDico = get_dico(dp, dutils,
                                "data/disease/test.txt",
                                "dictionary/disease.txt",
                                "Disease",
                                "Train", "disease")
    p_test_gene, r_test_gene, f1_test_gene, partial_test_gene, rpartial_test_gene, f1partial_test_gene, nb_notindico = dp.compute_metrics_generalize(newLabelsValid, newPredsValid, args.flag,1,inDico)  
    
    result_file = "results/" + args.num +"_final_results.txt"
    with open(result_file, "a+") as fw:
        print(args.lr, args.beta, args.gamma, args.drop_out,args.m, args.p,file=fw)
        print(p_best_train, r_best_train, f1_best_train, file=fw)
        print(p_train_partial, r_train_partial, f1_train_partial, partial_entity_train, partial_ratio_train,file=fw)

        print(best_precision_valid, best_rappel_valid,trainer.bestResult, file=fw) 
        print(p_valid_partial, r_valid_partial, f1_valid_partial, partial_entity_valid, partial_ratio_valid,file=fw)

        print(p_valid, r_valid, f1_valid, file=fw)
        print(p_test_partial, r_test_partial, f1_test_partial, partial_entity_test, partial_ratio_test,file=fw)
        
        print(p_test_gene, r_test_gene, f1_test_gene, file=fw)
        print(partial_test_gene, rpartial_test_gene, f1partial_test_gene, file=fw)
        print(nb_notindico,file=fw)
    if args.output:
        outputFile = "result/" + str(args.num)+"_feature_pu_" + args.dataset + "_" + args.flag + "_" +str(args.iter)+ ".txt"
    ...
    if args.output:
        outputFile = "result/" + str(args.num)+ "ada_pu_" + args.dataset + "_" + args.flag + "_"+str(args.iter)+ ".txt"
        with open(outputFile, "w") as fw:
            for i, sent in enumerate(test_words):
                preds = pred_test[i]
                probs = prob_test[i]
