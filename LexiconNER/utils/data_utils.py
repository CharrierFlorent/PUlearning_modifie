...
class DataPrepare(object):
        ...
        with open("data/glove.6B.100d.txt", "r", encoding='utf-8') as fw:
            for line in fw:
                ...
                if len(self.word2Idx) == 0:
                    ...
                    vector = np.zeros(len(splits) - 1)  # Zero vector or 'PADDING' word
        ...
        with open(filename, "r", encoding='utf-8') as fw:
            ...
            for line in fw:
                if len(line) == 0 or line.startswith('-DOCSTART') or line[0] == '\n':
                    ...
                    splits = line.split()
                    sentence.append([splits[0].strip(), splits[1].strip(), np.zeros(1)])
    ...
    def compute_precision_recall_f1(self, labels, preds, flag, pflag, partial=False):
        tp = 0
        np_ = 0
        pp = 0
        total = 0
        ...
        if partial:
          print('nb partial : ',pp-total, ' ratio entite complete :', total/pp)
        return p, r, f1


    def compute_precision_recall_f1_partial(self, labels, preds, flag, pflag, partial=False):
        tp = 0
        np_ = 0
        pp = 0
        total = 0
        ...
        if partial:
          print('nb partial : ',pp-total, ' ratio entite complete :', total/pp)
        return p, r, f1, pp-total, total/pp


    def get_words(self, dataset):
        words = {}
        if dataset == "conll2003" or dataset == 'conll2002' or dataset == 'disease':
        ...
        with open(filename, "r") as fw:
            ...
            for line in fw:
                if len(line) == 0 or line.startswith('-DOCSTART') or line[0] == '\n':
                    ...
                    splits = line.split()
                    sentence.append([splits[0].strip(), -1, np.zeros(1)])
    ...
    def compute_metrics_generalize(self, labels, preds, flag, pflag, inDico):
        tp = 0
        np_ = 0
        pp = 0
        partial_tp = 0
        ...
        for i in range(len(labels)):
            ...
            for j in range(len(sent_label)):
                ...
                for mots in range(item1.shape[0]):
                    if inDico[nb_mots+mots] == 1:
                        skip = True
                        break
                nb_mots += item1.shape[0]
                if skip:
                    skip = False
                    continue
                if (item1 == pflag).all() == True:
        ...
        if pp == 0:
            ...
            p = float(tp) / float(pp)
            partial_p = float(partial_tp) / float(partial_pp)
        if np_ == 0:
            r = 0
            partial_r = 0
        else:
            r = float(tp) / float(np_)
            partial_r = float(partial_tp) / float(partial_np_)
        if p == 0 or r == 0:
            f1 = 0
            partial_f1 = 0
        else:
            f1 = float(2 * p * r) / float((p + r))
            partial_f1 = float(2 * partial_p * partial_r) / float((partial_p + partial_r))
        return p, r, f1, partial_p,partial_r,partial_f1,np_
