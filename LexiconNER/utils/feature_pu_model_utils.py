...
class FeaturedDetectionModelUtils(ModelUtils):
    ...
    def add_dict_info(self, sentences, windowSize, datasetName):
        ...
        orgBigDic = set()
        miscBigDic = set()
        with open("feature_dictionary/" + datasetName + "/diseaseBigDic.txt", "r",encoding='utf-8') as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    perBigDic.add(line)
            ...
            for j, data in enumerate(sentence):
                ...
                for idx, word in enumerate(words):
                    ...
                    if word in perBigDic:
                        feature[self.dp.tag2Idx["Disease"]][count - 1] = 1
    ...
    def load_dataset(self, flag, datasetName, percent, num):
        fname = "data/" + datasetName + "/" + num + "_train." + flag + ".txt"
