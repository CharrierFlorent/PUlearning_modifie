...
def dict_match_word(dp, dutils, fileName, dictName, flag, mode, dataset, numero):
    ...
    print(count)
    if mode == "TRAIN":
        dp.writeFile("data/" + dataset + "/" + str(numero) + "_train." + flag + ".txt", mode, flag, sentences)
    ss = []
...
def get_dico(dp, dutils, fileName, dictName, flag, mode, dataset):
    sentences = dp.read_origin_file(fileName)
    length = [len(i) for i in sentences]
    maxLen = 10
    sentences, num, count = dutils.lookup_in_Dic(dictName, sentences, flag, maxLen)
